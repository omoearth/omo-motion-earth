<template>
  <div>
    <img src="/banner.png">
    <p class="has-text-centered has-text-weight-medium is-size-3 is-size-6-mobile has-text-dark">
      imagine your could jump on every
      <b class="has-text-weight-semibold">bike</b>,
      <b class="has-text-weight-semibold">scooter</b>,
      <b class="has-text-weight-semibold">car</b> and
      <b class="has-text-weight-semibold">public transport</b>
      in any place of the world all within one simple
      <br>
      <span class="is-size-2 is-size-5-mobile">
        <b class="has-text-primary has-text-weight-bold">green sharing flatrate</b>
      </span>
    </p>
  </div>
</template>

<script lang="ts">
import { Component, Vue } from "nuxt-property-decorator";

@Component({})
export default class OmoIllustration extends Vue {}
</script>
